...
function accountStats(acc,trades){
  ...
  /* running peak balance across sessions in date order (for trailing drawdown) */
  let running=size, peak=size;
  Object.keys(daily).sort().forEach(d=>{running+=daily[d];if(running>peak)peak=running;});
  ...
  let floor=-Infinity, locked=false;
  if(maxDD>0){
    if(ddType==='trailing'){ floor=peak-maxDD; }
    else if(ddType==='lock'){
      if(peak>=size+maxDD){ floor=size+lockBuffer; locked=true; }
      else floor=peak-maxDD;
    }
    else floor=size-maxDD; /* static: fixed from starting balance */
  }
...
const DD_TYPES=[
  ...
  {id:'trailing',label:'Trailing',hint:'Floor trails your highest balance, staying max-loss below your peak.'},
  {id:'lock',label:'Trailing + lock',hint:'Trails your peak, then locks once the buffer is met (e.g. 50K + 100 = 50,100 floor).'}
];
    ...
    <div className="grid g3">
      ...
      <div className="rule">
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          ...
          <span className="mono" style={{fontSize:11,color:'var(--dim)'}}>{s.maxDD?money(s.maxDD,0)+' · '+(s.ddType==='lock'?'trail+lock':s.ddType):'n/a'}</span></div>
        <div className="rulebar"><div style={{width:ddPct+'%',background:ddPct>75?'var(--dn)':ddPct>45?'var(--gold)':'var(--blu)'}}/></div>
...
function AccountModal({initial,onClose,onSave}){
  ...
  return <div className="ovl" onClick={onClose}><div className="mod" onClick={e=>e.stopPropagation()}>
    ...
    {mdd>0&&<div className="disc" style={{marginTop:-4,marginBottom:14}}>Starting floor: <b style={{color:'var(--ink)'}}>{money(floorPreview,0)}</b>{ddType!=='static'?' — then it '+(ddType==='lock'?'trails your peak, then locks.':'trails your peak upward.'):' — fixed.'}</div>}
...
For an ETF, adapt sensibly (expense ratio and top holdings in stats/financials, index level technicals). If a field is unknown use "n/a" or 0. CRITICAL: keep every string SHORT and the whole JSON under 400 words so it is COMPLETE and parseable.`;
      ...
      const t0=Date.now();
      const j=await fetchJSONCached(key,600000,prompt,3600,force===true);
      setCached(Date.now()-t0<250);
      setRes(normalizeResearch(j));setState('done');
